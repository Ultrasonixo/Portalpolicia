        ...
        const fetchData = async () => {
            try {
                const [statsResponse, anunciosResponse] = await Promise.all([
                    ...
                    fetch('http://localhost:3000/api/anuncios') // Busca os anúncios
                ]);
                ...
                <div className="column-right">
                    <div className="dashboard-widget">
                        ...
                        {loading ? (<p>Carregando anúncios...</p>) : 
                         anuncios.length > 0 ? (
                            <div className="anuncios-list-widget"> {/* Container para a lista */}
                                {anuncios.map(anuncio => (
                                    <div key={anuncio.id} className="anuncio-item-widget">
                                        <h4>{anuncio.titulo}</h4>
                                        <p className="anuncio-conteudo-widget">{anuncio.conteudo}</p>
                                        <p className="anuncio-meta-widget">
                                            Por <strong>{anuncio.autor_nome || 'Admin'}</strong> em {formatarData(anuncio.data_publicacao)}
                                        </p>
                                    </div>
                                ))}
                            </div>
